...
// ---------------------------------------------------------------------------------------------------------------------
const _replaceSpecialChar = (char) => {
  let toCompare = char.toUpperCase();

  if (['À', 'Á', 'Â', 'Ã', 'Å'].includes(toCompare) ) return 'A';
  if (['Ä', 'Æ'].includes(toCompare) ) return 'AE';
  if (['Ç', 'Č'].includes(toCompare) ) return 'C';
  if (['È', 'É', 'Ê', 'Ë'].includes(toCompare) ) return 'E';
  if (['Ì', 'Í', 'Î', 'Ï'].includes(toCompare) ) return 'I';
  if (['Ò', 'Ó', 'Ô', 'Õ'].includes(toCompare) ) return 'O';
  if (['Ö', 'Œ'].includes(toCompare) ) return 'OE';
  if (['Š'].includes(toCompare) ) return 'S';
  if (['ß'].includes(toCompare) ) return 'SS';
  if (['Ù', 'Ú', 'Û'].includes(toCompare) ) return 'U';
  if (['Ü'].includes(toCompare) ) return 'UE';
  if (['Ž'].includes(toCompare) ) return 'Z';
  //if (["'"].includes(toCompare) ) return '';
  //caratteri accentati trattati come vocali normali in lettura cf

  // TODO: AGGIUNGERE ALTRI RIMPIAZZAMENTI
  return char
};

const _replaceSpecialChars = (string) => {
  return string.split('').map(c => _replaceSpecialChar(c)).join('')
};
...
export const generateSurnamePart = (surname) => {
  ...
  surname = _replaceSpecialChars(surname);
  let result = getConsonants(surname);
  result = _fillIfNeeded(result, surname);
  return result.join('').substring(0, 3).toUpperCase();
};


export const generateNamePart = (name) => {
  name = name.toUpperCase();
  name = _replaceSpecialChars(name);
...
export const generatePartialTaxCode = (person) => {
  let result = generateSurnamePart(person.surname)
...
export const generateCin = (partialTaxCode = '') => {
  ...
  let odd = partialTaxCodeChars.filter((char, index) => index <= 14 && (index + 1) % 2 === 1);
  let even = partialTaxCodeChars.filter((char, index) => index <= 14 && (index + 1) % 2 === 0);
...
export const generateAllParts = (person) => {
  return {
    ...
    birthDayAndGender: generateBirthDayAndGenderPart(person.birthDay, person.gender),
...
export const extractBirthDayAndGenderPart = (taxCode = '') => {
  taxCode = taxCode.toUpperCase();
  return taxCode.substring(DAY_AND_GENDER_INDEX_START, DAY_AND_GENDER_INDEX_END + 1);
};
...
export const getLevelAlternativeTaxCodes = (taxCode = '', level = 1) => {
  if (level <= 0 || level > OMOCODIA_MAX_LEVEL) console.warn('Invalid omocodia level as argument');
  let alternatives = getAlternativeTaxCodes(taxCode, level);
  return alternatives[level];
};


export const getAlternativeTaxCodes = (taxCode = '', maxLevel = OMOCODIA_MAX_LEVEL, flat = false) => {
  ...
  for (let level = 2; level <= maxLevel; level++) {
    ...
    previousLevel.forEach(taxCode => {
      result[level] = result[level].concat(_getOneLevelAlternativeTaxCodes(taxCode));
    });
  }
...
export const getOmocodiaLevel = (taxCode = '') => {
  let partialTaxCode = extractPartialTaxCode(taxCode);
  let digitCounts = partialTaxCode.split('').filter(char => _isDigit(char)).length;
