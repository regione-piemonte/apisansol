...
module.exports = function (/* ctx */) {
  ...
  let boot = ["config", "http", "filters", "globals", "rules"];

  // Matomo lo aggiungiamo solo in ambiente di test e di produzione
  if (!envs.APP_IS_DEV) {
    boot.push("matomo");
  }
  ...
  return {
    ...
    framework: {
      ...
      // Quasar plugins
      plugins: ["Notify", "Dialog", "SessionStorage", "Cookies"],
    },
    ...
    devServer: {
      ...
      proxy: {
        "/api-public/": {
          ...
          changeOrigin: true,
          secure: false,
          pathRewrite: { "^/api-public/": "/public/index.php/" },
        },
        "/api/": {
          target: "http://localhost:8000",
          changeOrigin: true,
          secure: false,
          pathRewrite: { "^/api/": "/auth/index.php/" },
        },
