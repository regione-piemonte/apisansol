...
export const setCmsMessageList = (context, { messageList }) => {
  context.commit("SET_CMS_MESSAGE_LIST", { messageList });
};
...
export const startOtpSession = (context) => {
  Cookies.set("KEY", true, { path: "/" });
  context.commit("SET_OTP_LOGIN", { login: true });
};

export const closeOtpSession = (context) => {
  Cookies.remove("KEY", { path: "/" });
  context.commit("SET_OTP_LOGIN", { login: false });
};

// Se la sessione OTP è già attiva => la rinfreschiamo
export const checkOtpSession = (context) => {
  let isOtpLoginActive = Cookies.get("KEY");
  if (isOtpLoginActive) context.dispatch("startOtpSession");
};

export const setCitizen = (context, { citizen }) => {
  context.commit("SET_CITIZEN", { citizen });
};

export const setCitizenError = (context, { error }) => {
  context.commit("SET_CITIZEN_ERROR", { error });
};

export const addDocumentInfo = (
  context,
  { departmentCode, documentInfo, error }
) => {
  let documentInfoList = context.getters["getDocumentInfoList"];
  documentInfo = documentInfo || {};
  documentInfoList = [
    ...documentInfoList,
    { codice_dipartimentale: departmentCode, ...documentInfo, errore: error },
  ];
  context.commit("SET_DOCUMENT_INFO_LIST", { documentInfoList });
};

export const setUserApplicationsList = (context, { appList }) => {
  context.commit("SET_USER_APPLICATIONS_LIST", { appList });
};
