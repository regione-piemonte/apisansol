...
export const isDocumentPayed = (paymentStatus) => {
  ...
  let hasConsents =
    patient?.consensi?.consenso_consultazione &&
    patient?.consensi?.consenso_alimentazione;
...
export const getDocumentFseTypeList = (validDocTypes) => {
  ...
  let categoryList = deepClone(
    categories?.filter((c) => c.codice_categoria === DOCUMENT_CATEGORY_MAP.FSE)
  );
...
export const getDocumentFse = async (payload) => {
  ...
  try {
    ...
    let message = `Non è stato possibile recuperare il referto ${
      payload.id_documento_ilec ?? ""
    }`;
...
export const getDocumentPersonal = async (id) => {
  ...
  try {
    ...
    let message = `Non è stato possibile recuperare il documento ${id}`;
    apiErrorNotify({ error, message });
    return null;
  }
};
export const getDocumentExtraRegion = async (payload) => {
  ...
  if (!payload) return null;
  try {
    let { data: document } = await downloadDocumentNoPiedmont(
      patientTaxCode,
      payload
    );
    return document;
