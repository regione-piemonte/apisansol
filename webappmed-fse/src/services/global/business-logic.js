...
import { GENDER_OPTIONS, SYSTEMS_CODE_MAP } from "src/services/global/config";
...
// Controlliamo che non abbia un versione dell'informativa scaduta . Questo controllo non viene fatto se il regime è assistenza
export const consentsVersionControl = (consents) => {
  const currentInformative =
    consents?.identificativo_informativa_corrente ?? "";
  let currentInformativeRegion = currentInformative.slice(0, 3);
  let currentInformativeCode = currentInformative.slice(
    currentInformative.lastIndexOf("^") + 1
  );
  const consentInformative =
    consents?.identificativo_informativa_consensi ?? "";
  let consentInformativeRegion = consentInformative.slice(0, 3);
  let consentInformativeCode = consentInformative.slice(
    consentInformative.lastIndexOf("^") + 1
  );
  ...
  return (
    isEmergencySystem ||
    currentInformativeCode === consentInformativeCode ||
    currentInformativeRegion !== consentInformativeRegion
  );
};
