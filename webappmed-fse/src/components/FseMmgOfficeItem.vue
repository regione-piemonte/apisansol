  ...
  <div v-if="office" class="q-pa-lg row items-start q-col-gutter-md">
    ...
    <q-item class="col-12 col-md-6">
      <q-item-section>
        <q-item-label caption class="text-black"
          >Numero di telefono</q-item-label
        >
    ...
    <q-item class="col-12 col-md-6">
      <q-item-section>
        ...
        <q-item-label
          v-if="officeHoursList.length <= 0"
          class="text-italic text-caption text-accent"
        >
          Orario non disponibile
        </q-item-label>
        <template v-else>
          <q-item-label
            v-for="(hour, index) in officeHoursList"
            :key="index"
...
export default {
  ...
  computed: {
    ...
    addressLabel() {
      if (this.address) {
        return `${this.address.indirizzo ?? ""} - ${this.address.cap ?? ""} ${
          this.address.desc_comune ?? ""
        }`;
      } else {
        return "";
      }
