...
<template>
  <q-card>
    ...
    <q-expansion-item
      default-opened
      v-if="visualizationMode === VISUALIZATION_MAP.TABLE"
    >
      <template #header>
...
const PRESSURE_DESCRIPTORS = [
  DESCRIPTOR_CODE_MAP.DIASTOLIC_PRESSURE,
  DESCRIPTOR_CODE_MAP.SYSTOLIC_PRESSURE,
];
...
export default {
  ...
  props: {
    ...
    visualizationMode: {
      type: [String, Number],
      default: VISUALIZATION_MAP.TABLE,
    },
  },
  data() {
    return {
  ...
  methods: {
    async getDetectionsList() {
      ...
      let promisePresMin = await getNotebookDetectionList(
        this.patientTaxCode,
        this.sanitaryNoteBookId,
        payloadPresMin,
        { params }
      );
      ...
      let promisePresMax = await getNotebookDetectionList(
        this.patientTaxCode,
        this.sanitaryNoteBookId,
        payloadPresMax,
        { params }
      ...
      try {
        ...
        let pressMinDetectionList =
          pressMinDetectionResponse?.elenco_rilevazioni ?? [];
        let pressMaxDetectionList =
          pressMaxDetectionResponse?.elenco_rilevazioni ?? [];
        this.detectionPressMinList = [
          ...this.detectionPressMinList,
          ...pressMinDetectionList,
        ];
        this.detectionPressMaxList = [
          ...this.detectionPressMinList,
          ...pressMaxDetectionList,
        ];

        this.offset = this.detectionPressMinList.length;

        this.detectionsTotalCount =
          pressMinDetectionResponse?.numero_rilevazioni ?? 0;
      } catch (error) {
        let message = `Non è stato possibile recuperare le rilevazioni di ${this.descriptorName}`;
