...
export default {
  ...
  methods: {
    async getDetectionsList() {
      ...
      let insulinPromise = await getNotebookDetectionList(
        this.patientTaxCode,
      ...
      let insulinFarmPromise = await getNotebookDetectionList(
        this.patientTaxCode,
        this.sanitaryNoteBookId,
        payloadInsulinFarm,
        { params }
      ...
      try {
        ...
        let insulinDetectionList =
          insulinDetectionResponse?.elenco_rilevazioni ?? [];
        let insulinFarmDetectionList =
          insulinFarmDetectionResponse?.elenco_rilevazioni ?? [];
        this.detectionInsulinList = [
          ...this.detectionInsulinList,
          ...insulinDetectionList,
        ];
        this.detectionInsulinFarmList = [
          ...this.detectionInsulinFarmList,
          ...insulinFarmDetectionList,
        ];
        this.offset = this.detectionInsulinList.length;
        this.detectionsTotalCount =
          insulinDetectionList?.numero_rilevazioni ?? 0;
      } catch (error) {
