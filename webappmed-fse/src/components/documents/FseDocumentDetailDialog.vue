  ...
  >
    <q-card style="min-width: 80%">
      <q-toolbar class="q-mb-sm q-px-lg">
        <q-toolbar-title> Dettaglio referto #{{ documentId }} </q-toolbar-title>
        <q-btn
          v-close-popup
            ...
            <template #secondary>
              <div class="q-pa-lg row items-start q-col-gutter-md">
                ...
                <q-item class="col-12 col-md-6">
                  <q-item-section>
                    <q-item-label caption class="text-black"
                      >Assetto organizzativo</q-item-label
                    >
                    <q-item-label class="text-bold">{{
                      organizationalStructure | empty
                    }}</q-item-label>
                  </q-item-section>
                </q-item>
                <template v-if="mediationDate">
                  ...
                  <q-item class="col-12 col-md-6">
                    <q-item-section>
                      <q-item-label caption class="text-black"
                        >Data mediazione</q-item-label
                      >
                      <q-item-label class="text-bold">{{
                        mediationDate | date | empty
                      }}</q-item-label>
                    </q-item-section>
                  </q-item>
                  ...
                  <q-item class="col-12 col-md-6">
                    <q-item-section>
                      <q-item-label caption class="text-black"
                        >Autore mediazione</q-item-label
                      >
                      <q-item-label class="text-bold">{{
                        mediationAuthor | empty
                      }}</q-item-label>
                    </q-item-section>
                  </q-item>
                </template>
      ...
      <template v-else>
        <csi-banner type="warning" class="q-ma-lg">
          Non è stato possibile recuperare il referto.
        </csi-banner>
      </template>
            ...
            <q-item class="q-pb-sm">
              <q-item-section>
                <q-item-label class="text-h6">{{
                  typeCode === "97500-3"
                    ? "Motivazione certificati verde Covid"
                    : "Prestazioni"
                }}</q-item-label>
              </q-item-section>
            </q-item>
            ...
            <q-item v-for="(service, index) in healthServices" :key="index">
              <q-item-section>
                <q-item-label
                  ...
                  <span v-if="service.data_prestazione"
                    >&nbsp; ({{
                      service.data_prestazione | date("DD/MM/YYYY")
                    }})</span
                  ></q-item-label
                >
              </q-item-section>
...
export default {
  ...
  computed: {
    ...
    doctors() {
      ...
      doctors = doctors.map((doctor) => {
        ...
        let type = doctor.tipo_medico
          ? DOCUMENT_DOCTOR_TYPES_LABEL[doctor.tipo_medico]
          : null;
    ...
    signIcon() {
      return this.sign
        ? `img:icone/documenti/icon-${
            DOCUMENT_SIGN_CODE_MAP_ICON[this.sign]
          }.svg`
        : null;
    },
  },
  async created() {
    if (!this.document) {
      ...
      let documentPromise = getDocumentDetail(
        this.patientTaxCode,
        this.documentId,
        { params }
      );
      ...
      if (this.documentCategory === DOCUMENT_CATEGORY_MAP.FSE) {
        ...
        healthServicePromise = getDocumentHealthServices(
          this.patientTaxCode,
          this.documentId,
          { params }
        );
      ...
      try {
        ...
        let message = "Non è stato possibile recuperare il documento.";
      ...
      if (healthServicePromise) {
        try {
          ...
          let message =
            "Non è stato possibile recuperare la lista delle prestazioni.";
          apiErrorNotify({ error, message });
        }
      }
