...
<template>
  <div
    :class="[
      ($q.screen.lt.md && listView) || (detail && $q.screen.gt.sm)
        ? 'items-end'
        : 'items-stretch',
    ]"
  ...
  >
    <template v-if="isDocumentMediated">
      <div class="row items-center justify-center">
        <div class="text-green-7 text-bold">Documento mediato</div>
      </div>
    </template>
    ...
    <template v-if="isPersonal">
      <q-btn
        v-if="
          contributionType ===
          DOCUMENT_PERSONAL_CONTRIBUTION_TYPE_MAP.TRANSCRIPTION
        "
        :to="transcriptionUrl"
        color="primary"
        outline
    ...
    <template v-else-if="isFse">
      ...
      <template v-if="needMediation">
        <q-btn
          ...
          color="primary"
          outline
          @click.stop="showMediationDialog = true"
        >
          Media documento
        </q-btn>
      </template>
      ...
      <template v-else-if="isPayed">
        <q-btn
          v-if="canBookingImage"
          :loading="isBookingImage"
          color="primary"
          outline
          @click.stop="onBookImage"
        >
          Prenota immagine
        </q-btn>
        ...
        >
          <q-list>
            <q-item
              v-close-popup
              clickable
              @click.stop="showDownloadImageDialog = true"
            >
              <q-item-section>
    ...
    <q-dialog v-model="showMediationDialog" :maximized="$q.screen.lt.md">
      <q-card style="max-width: 800px">
        <q-toolbar>
          <q-toolbar-title> Mediazione </q-toolbar-title>
          <q-btn
            v-close-popup
        ...
        <q-card-section>
          <csi-banner type="warning">
            Non sarà possibile rioscurare un documento al paziente una volta
            confermata l’avvenuta mediazione.
          </csi-banner>

          <csi-buttons class="q-mt-md">
            <csi-button
              :loading="isLoadingMediation"
              @click.stop="onConfirmMediation"
              >Confermo mediazione</csi-button
            >
    ...
    <q-dialog ref="downloadImageDialog" v-model="showDownloadImageDialog">
      <q-card style="max-width: 800px">
        <q-toolbar>
        ...
        <q-card-section class="q-gutter-y-md">
          <csi-banner class="q-mt-md" type="info">
            ...
            Il file contenente le immagini potrebbe avere dimensioni
            considerevoli e richiedere lunghi tempi di attesa.
          </csi-banner>
...
export default {
  ...
  computed: {
    ...
    canBookingImage() {
      return (
        this.accessionNumbers?.length > 0 &&
        this.metaData?.codice_tipo_documento ===
  ...
  methods: {
    async onConfirmMediation() {
      ...
      try {
        let response = await setDocumentMediation(
          this.patientTaxCode,
          this.id,
          { params }
        );
        ...
        let message =
          "Non è stato possibile mediare il documento. Riprovare più tardi";
    ...
    async downloadDocumentPersonal() {
      ...
      try {
        let { data: document } = await downloadDocumentPersonal(
          this.patientTaxCode,
          this.id
        );
