...
<template>
  ...
  >
    <q-card :class="classes" class="fse-associate-list-dialog--card">
      <q-toolbar>
        <q-toolbar-title>
          Documenti associati per
          {{ ASSOCIATION_OPERATION_TYPE_LABEL_MAP[this.operationType] }}
        </q-toolbar-title>
      ...
      <q-card-section>
        <div
          :class="$q.screen.gt.sm ? 'items-stretch' : 'items-start'"
        ...
        >
          ...
          <template v-else>
            <div
              v-for="document in orderedDocumentList"
              :key="'document-association--' + document.id_documento_ilec"
...
export default {
  ...
  components: {
    ...
    FseDocumentItem: () => import("./FseDocumentItem"),
  },
  props: {
  ...
  computed: {
    ...
    orderedDocumentList() {
      const IMPORTANT_CODE_TYPES = ["1", "2"];
      ...
      for (let i = 0; i < list.length; i++) {
        let doc = list[i];
        if (
          IMPORTANT_CODE_TYPES.includes(
            doc.metadati_documento?.codice_tipo_documento
          )
        ) {
          list.splice(i, 1);
          list.unshift(doc);
  ...
  async created() {
    ...
    try {
      let { data } = await getDocumentAssociatedList(
        this.patientTaxCode,
        this.documentId,
        {
          params,
      ...
      this.documentList = documentList?.filter(
        (doc) => doc.id_documento_ilec !== this.documentId
      );
    } catch (error) {
      let message = `Non è stato possibile caricare i documenti associati per ${
        ASSOCIATION_OPERATION_TYPE_LABEL_MAP[this.operationType]
      }`;
