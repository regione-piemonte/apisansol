...
<template>
  <div
    :class="classes"
    class="fit fse-document-item"
    v-on:click.stop="onClickDocument"
  >
    <q-card class="fit">
      <!-- VISTA LISTA -->
      <!-- ----------------------------------------------------------------------------------------------------------- -->
      ...
      <q-card-section v-if="listView" class="no-padding bg-white">
        ...
        <template v-if="!isPayed">
          <csi-card-item-bar type="warning">
            Non è possibile consultare questo referto per mancato pagamento del
            ticket
          </csi-card-item-bar>
        </template>
        ...
        <template v-if="needMediation">
          <csi-card-item-bar type="warning">
            Il paziente non può visualizzare questo referto fino a mediazione
            avvenuta
          </csi-card-item-bar>
        </template>
        <!-- DOCUMENTO -->
        ...
        <div class="row">
          <div
            class="row items-start bg-grey-3 q-py-md q-px-md col-12 col-md-3 col-lg-2"
          >
            <div
              v-if="isSelectable"
              class="col-auto self-start"
              :class="{ 'q-pr-md': $q.screen.lt.md }"
            >
              <q-checkbox
            ...
            <div class="col row items-center">
              <div
                :class="[
                  { 'text-center': $q.screen.gt.xs },
                  { 'q-pr-md': $q.screen.lt.md },
                ]"
                class="col-auto col-md-12"
              >
          ...
          <div class="row items-start q col-12 col-md q-pa-md">
            <div class="col-12 col-md row items-start q-col-gutter-md">
              ...
              <q-item class="col-12 col-md-4">
                <q-item-section>
                  <q-item-label caption class="text-black"
                    >Emesso il</q-item-label
                  >
                  <q-item-label class="text-bold">{{
                    issueDate | date | empty
                  }}</q-item-label>
                </q-item-section>
              </q-item>
            ...
            <div class="col-12 col-md-3">
              <div class="q-my-md">
                <a
                  v-if="!isNoPiedmontPatient"
                  class="csi-link self-center"
                  href="#"
                  @click.prevent="showDocumentDetail"
                >
        ...
        <csi-card-item on-side>
            ...
            >
              ...
              <template #iconright>
                <template v-if="showLockIcon">
                  <q-icon
                    v-if="isDocumentMediated"
                    name="lock_open"
                    size="sm"
                  />
                  <q-icon
                    v-if="needMediation || !isPayed"
          ...
          <template #secondary>
            <div class="full-height column justify-between">
              ...
              <template v-if="!isPayed">
                <csi-card-item-bar type="warning">
                  Non è possibile consultare questo referto per mancato
              ...
              <template v-if="needMediation">
                <div class="col-auto">
                  <csi-card-item-bar type="warning">
                    Il paziente non può visualizzare questo referto fino a
                    mediazione avvenuta
                  </csi-card-item-bar>
                </div>
              </template>
              ...
              <div class="q-px-lg q-py-md row q-col-gutter-md">
                <a
                  v-if="!isNoPiedmontPatient"
                  class="csi-link col-12"
                  href="#"
                  @click.prevent.stop="showDocumentDetail"
                >
                  <span class="text-bold">Maggiori informazioni</span>
                </a>
          ...
          <template #actions>
            <fse-document-actions
              :document="document"
              class="q-mx-md q-mb-md"
              @on-mediated="onMediated"
            />
          </template>
        </csi-card-item>
      </q-card-section>
...
export default {
  ...
  computed: {
    ...
    patientHasConsents() {
      return (
        this.patientConsents?.consenso_consultazione &&
        this.patientConsents?.consenso_alimentazione
      );
    },
    patientConsents() {
    ...
    hasHospitalizationAssociated() {
      return (
        this.metaData?.flag_episodi_collegati &&
        this.document?.numero_nosologico
      );
    },
    nosological() {
    ...
    hasPsAssociated() {
      return (
        this.metaData?.flag_episodi_collegati && this.document?.passaggio_ps
      );
    },
    hasNreAssociated() {
  ...
  methods: {
    async getMediationInfo() {
      try {
        let { data } = await getDocumentMediationInfo(
          this.patientTaxCode,
          this.id
        );
