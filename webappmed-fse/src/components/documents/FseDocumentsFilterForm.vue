    ...
    <q-card-section class="no-padding">
      ...
      <q-slide-transition>
        <div v-show="isOpenFilters" class="q-pb-sm">
          <div
            class="row items-center q-col-gutter-x-lg q-col-gutter-y-md q-mb-md"
          >
...
export default {
  ...
  computed: {
    doctorTypeOptions() {
      ...
      if (this.doctorReports) {
        list = list.filter(
          (type) => type.value !== DOCUMENT_DOCTOR_TYPE_MAP.BOTH
        );
      }
    ...
    filterDocumentCategoryCodeList() {
      return this.filterCategory
        ? [this.filterCategory?.codice_categoria]
        : this.categoryList.flatMap((c) => c.codice_categoria);
    },
    documentTypeOptions() {
      let docTypeList = this.documentTypeList ?? [];

      docTypeList = docTypeList.filter((d) =>
        this.filterDocumentCategoryCodeList.includes(d.codice_categoria)
      );

      let result = uniqueElementsBy(
        docTypeList,
        (a, b) => a.codice === b.codice
      );
  ...
  created() {
    this.filterPeriod = PERIOD_LIST_FILTER.find(
      (p) => p.codice === this.startPeriodCode
    );

    if (this.defaultDocumentType) {
      this.filterDocumentType = this.documentTypeOptions.filter(
        (type) => type.codice === this.defaultDocumentType
      );
  ...
  methods: {
    ...
    setFilters() {
      let startDate = this.filterPeriod?.startDate
        ? formatDate(this.filterPeriod?.startDate, FORMAT_DATE)
        : null;
      let endDate = this.filterPeriod?.endDate
        ? formatDate(this.filterPeriod?.endDate, FORMAT_DATE)
        : null;
