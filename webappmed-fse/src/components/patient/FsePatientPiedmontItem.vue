...
<template>
  <q-card class="fit cursor-pointer">
    <q-card-section
      class="column q-pa-none items-stretch justify-between full-height"
    >
      ...
      <div class="col full-height q-pa-md">
        <div class="row items-start q-col-gutter-lg">
          ...
          <div class="col-12 col-md-6">
            ...
            <div>
              <strong>{{ gender | empty }}</strong>
            </div>
          </div>
          <div class="col-12 col-md">
            <div class="text-caption">Luogo di nascita</div>
            <div>
              <strong>{{ birthPlace | empty }}</strong>
            </div>
          </div>
        </div>
      </div>
      <div class="q-pa-md col-auto">
        <csi-buttons>
          <csi-button :loading="isLoading" no-min-width @click="onSelectPatient"
            >Vai al fascicolo</csi-button
          >
        </csi-buttons>
      </div>
    </q-card-section>
    <!--DIALOGS-->
    <!-- ---------------------------------------------------------------------------------------------------  -->
    <fse-dichiaration-consent-dialog
      v-model="showDichiarationDialog"
      :consents="consents"
      :patient="patient"
...
export default {
  ...
  computed: {
    icon() {
      let patientIcon =
        this.patient?.sesso === "F" ? "avatar-donna" : "avatar-uomo";
    ...
    gender() {
      let gender = "";
      if (this.patient?.sesso === "F") gender = "Femmina";
      else gender = "Maschio";
  ...
  methods: {
    async onSelectPatient() {
      ...
      if (this.isObsoleteVersion) {
        ...
        this.consents?.consenso_alimentazione &&
        this.consents?.consenso_consultazione
      ) {
        this.showDichiarationDialog = true;
      } else {
        this.showConsentsDialog = true;
    ...
    async onConfirmConsentDichiaration() {
      ...
      try {
        ...
        let message = "Non è stato possibile confermare il consenso";
