...
<template>
  ...
  >
    <q-card style="max-width: 800px">
      <q-toolbar>
        <q-toolbar-title> Consensi </q-toolbar-title>
        <q-btn
          v-close-popup
      ...
      <q-card-section>
        ...
        <template v-else-if="isObsoleteVersion">
          <p>
            Poichè l'informativa è stata aggiornata è necessario che l'assistito
            prenda visione della nuova versione del documento e comunichi
            nuovamente il valore del consenso alla consultazione.
          </p>
        </template>
        <template v-else>
          <p>Non è possibile accedere al Fascicolo sanitario del paziente.</p>
          ...
          <p v-else>
            Il paziente non ha il Fascicolo Sanitari attivo. L'assistito
            desidera aprire il fascicolo?
          </p>
        </template>

        <p v-if="isEmergencySystem">
          Puoi consultare i referti che hai redatto/validato
          <span v-if="!hidePSS">e il PSS</span>
        </p>
        <p v-else>Puoi consultare i referti che hai redatto/validato</p>
        <csi-buttons class="q-mt-md">
          <template v-if="!consentError">
            <csi-button @click="$emit('consents-activation')">{{
              goToConsentsText
            }}</csi-button>
          </template>
          <csi-button @click="$emit('consult-reports')"
            >Consulta i referti</csi-button
          >
