...
<template>
  <div>
    ...
    <template v-else>
      <csi-page-title no-back class="q-mb-lg"
        >Profilo di Accesso</csi-page-title
      >
      ...
      <template v-else-if="rolesOptions.length > 4 && $q.screen.gt.sm">
        <csi-scroll-horizontal controls draggable>
                ...
                <q-card-section class="items-center full-height" horizontal>
                  ...
                  <q-card-section class="no-padding">
                    <div class="text-subtitle2">
                      <strong>{{ role.descrizione }}</strong>
                    </div>
                  </q-card-section>
                </q-card-section>
              </q-card>
      ...
      <div v-else class="row items-center q-col-gutter-lg q-pb-md">
        <div
          ...
          :key="index"
          class="col-12 col-sm-6 col-md-3 items-stretch"
        >
          <q-card
            :class="{ active: isActiveRole(role.codice) }"
            class="bg-white user-role-system-card q-pa-md fit"
      ...
      <template v-if="selectedRole">
        ...
        <h2 class="text-h5 text-bold q-my-lg">Collocazione</h2>
        ...
        <template v-else>
          <csi-banner v-if="placementsOptions.length === 0" type="info">
            Non ci sono collocazioni disponibili per il profilo di accesso
            selezionato
          </csi-banner>
          <q-card v-else class="q-mb-lg">
            <q-card-section>
              <q-list>
                <q-item
                  v-for="(placement, index) in placementsOptions"
                  :key="index"
                ...
                >
                  <q-item-section side top>
                    <csi-radio-button
                      ...
                      :value="index === selectedPlacementIndex"
                  ...
                  <q-item-section>
                    ...
                    <q-item-label>{{
                      placement.descrizione_collocazione
                    }}</q-item-label>
                  </q-item-section>
                </q-item>
              </q-list>
        ...
        <template v-if="selectedPlacement">
          <h2 class="text-h5 text-bold q-my-lg">Regime</h2>
          <div
            class="row items-center q-col-gutter-lg q-pb-md"
            v-if="isLoadingSystems"
          >
                ...
                <div v-for="(system, index) in systemOptions" :key="index">
                  ...
                  >
                    <q-card-section class="items-center full-height" horizontal>
                      ...
                      <q-card-section class="no-padding">
                        <div class="text-subtitle2">
                          <strong>{{ system.descrizione }}</strong>
                        </div>
                      </q-card-section>
                    </q-card-section>
                  </q-card>
          ...
          <div v-else class="row items-center q-col-gutter-lg q-pb-md">
            <div
              v-for="(system, index) in systemOptions"
              :key="index"
              class="col-12 col-sm-6 col-md items-stretch"
            >
              <q-card
                :class="{ active: isSelectedSystem(system.codice) }"
                class="bg-white user-role-system-card q-pa-md fit"
          ...
          <csi-buttons v-if="selectedSystem" class="q-my-lg">
            <csi-button @click="$emit('on-confirm')" :loading="confirming"
              >Prosegui</csi-button
            >
          </csi-buttons>
        </template>
  ...
  computed: {
    ...
    placementsOptions() {
      ...
      return this.placementsList.map((placement) => {
        placement.label_code = `${
          placement.codice_azienda
        } - ${placement.codice_collocazione?.replaceAll("@", " ")}`;
        return placement;
      });
    },
  },
  async created() {
    ...
    if (!this.hideRoleList) {
      try {
        ...
        let message =
          "Non è stato possibile recuperare l'elenco dei profili di accesso.";
  ...
  methods: {
    ...
    async showPlacements(role) {
      ...
      try {
        ...
        this.placementsList = [];
        let message =
          "Non è stato possibile recuperare l'elenco delle collocazioni per il profilo di accesso selezionato.";
        apiErrorNotify({ error, message });
      } finally {
    ...
    onSelectPlacement(placement, index) {
      ...
      if (isEmpty(this.systemList)) this.getSystemList();
    },
    ...
    async getSystemList() {
      ...
      try {
        ...
        let message = "Non è stato possibile recuperare la lista dei regimi";
        apiErrorNotify({ error, message });
      }
    ...
    getRoleIconMap(code, gender) {
      ...
      if (GENERIC.includes(roleCode)) role = ICONS_MAP.GENERIC;
      else if (MANAGER.includes(roleCode)) role = ICONS_MAP.MANAGER;
      else if (OPERATOR.includes(roleCode)) role = ICONS_MAP.OPERATOR;
      else if (DOCTOR.includes(roleCode)) role = ICONS_MAP.DOCTOR;
      else if (PHARMACY.includes(roleCode)) role = ICONS_MAP.PHARMACY;
      else if (NURSE.includes(roleCode)) role = ICONS_MAP.NURSE;
      else role = ICONS_MAP.OPERATOR;
    ...
    normalizePlacementNames(placements) {
      return placements?.map((placement) => {
        placement.descrizione_collocazione =
          placement.descrizione_collocazione.replaceAll("@", " ");
        return placement;
      });
    },
