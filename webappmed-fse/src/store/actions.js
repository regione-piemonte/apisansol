...
export const setCategoriesList = async (context) => {
  ...
  try {
    ...
    let message = "Non è stato possibile recuperare la lista delle categorie."
    apiErrorNotify({error, message})
  }
}
  ...
  try {
    ...
    if (error.response.status === 400) {
      ...
      if (errorKey === '2035') {
        ...
        //Se l'assistito è deceduto/emigrato o cf cancellato:
    ...
    let message = "Non è stato possibile recuperare lo stato dei consensi del paziente"
    apiErrorNotify({error, message})
...
export const getDocumentList = async (context, payload) => {
  ...
  try {
    ...
    let message = "Non è stato possibile recuperare la lista dei documenti."
    apiErrorNotify({error, message})
    return null
  }
