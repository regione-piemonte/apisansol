      ...
      <template v-slot:before>
        <template v-if="isMobile">
          ...
          >
            ...
            >
              <fse-tac-menu-item
                :is-selected="isMenuItemSelected(menuItem)"
                :menu-item="menuItem"
        ...
        <template v-else>
          <div
            ref="tacDetectionsMenuContainer"
            class="row items-center q-pb-lg justify-center"
            style="margin: 0 auto"
          >
            <q-list
              id="fse-tac-detections--container "
              :class="animationCompleted ? 'col-12' : 'col-8'"
      ...
      <template v-slot:after>
        <div class="q-mx-md" :class="{ 'q-mt-lg': isMobile }">
          <template v-if="menuItemSelected">
            ...
            <div
              v-for="descriptor in menuItemSelected.descriptors"
              :key="descriptor"
              class="q-my-lg"
            >
...
const MENU_OPTIONS = [
  {
    ...
    descriptors: [
      DESCRIPTOR_CODE_MAP.WEIGHT,
      DESCRIPTOR_CODE_MAP.WAIST_CIRC,
      DESCRIPTOR_CODE_MAP.HEIGHT,
    ],
  },
  {
    name: "Colesterolo",
    ...
    descriptors: [
      DESCRIPTOR_CODE_MAP.CHOLESTEROL_HDL,
      DESCRIPTOR_CODE_MAP.CHOLESTEROL_LDL,
      DESCRIPTOR_CODE_MAP.TRIGLYCERIDES,
      DESCRIPTOR_CODE_MAP.CHOLESTEROL_TOT,
    ],
  },
  {
    name: "Emoglobina",
...
const TABLE_COLS = [
  ...
  },
  {
    ...
    label: "Misura",
    field: (row) =>
      row.valore_testuale ? row.valore_testuale : row.valore_numerico,
    format: (val) => (val ? `${val}` : "-"),
    align: "left",
    sortable: true,
  },
...
export default {
  ...
  computed: {
    ...
    menuOptions() {
      ...
      let detection = this.sanitaryNotebook?.preferenze.find(
        (p) => p.codice === "RILEVAZIONI"
      );
  ...
  methods: {
    scrollToSelectedDetection() {
      let seletedItem = this.menuOptions?.find(
        (item) => item.id == this.menuItemSelected?.id
      );
    ...
    onSelectDesktop(menuItem, code, index) {
      ...
      if (!this.animationCompleted) {
        ...
        let container =
          this.$refs.tacDetectionsMenuContainer.getBoundingClientRect().left;
