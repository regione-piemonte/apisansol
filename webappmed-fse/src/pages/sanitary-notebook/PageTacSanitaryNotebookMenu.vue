...
<template>
  <csi-page padding style="margin: 0 auto">
    <div
      ...
      class="row justify-center items-stretch q-col-gutter-lg"
    >
    ...
    <template v-else-if="noNotebookErr">
      <csi-banner type="warning">
        Taccuino sanitario non consultabile perché non è mai stato creato
      </csi-banner>
    </template>

    <div
      v-else-if="sanitaryNotebook"
      :class="{ reverse: $q.screen.gt.sm }"
      class="row justify-center items-stretch q-col-gutter-lg"
    >
      <div class="col-12 col-md-3">
      ...
      <div class="col-12 col-md">
        <div class="row items-center q-col-gutter-md">
          ...
          <template v-else>
            ...
            >
              <fse-tac-menu-item
                :is-selected="menuItemSelected === menuItem"
                :menu-item="menuItem"
                @on-select="onSelect"
              />
            </div>
    ...
    <template v-else>
      <csi-banner type="warning"> Taccuino sanitario non trovato </csi-banner>
    </template>
  ...
  computed: {
    ...
    menuOptions() {
      ...
      for (let i = 0; i < menuList.length; i++) {
        ...
        let activePreference = preferences?.find(
          (p) => p.codice === menuItem.id
        );
        if (activePreference) {
    ...
    if (!this.sanitaryNotebook) {
      try {
        ...
        if (error.response.status === 404) {
          ...
          let message =
            "Non è stato possibile recuperare il taccuino sanitario";
