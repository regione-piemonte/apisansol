      ...
      <q-card-section class="no-padding">
        ...
        <q-slide-transition>
          <div v-show="isOpenFilters" class="q-pb-sm">
            <div
              class="row items-end q-col-gutter-x-lg q-col-gutter-y-md q-mb-md"
            >
          ...
          >
            <template #header>
              <q-item-section>
                ...
                <template v-if="episode.unita_operativa_accettazione">
                  {{ episode.unita_operativa_accettazione }}
                  -
                </template>

                <template v-if="episode.struttura_accettazione">
  ...
  methods: {
    ...
    async loadEpisodesList() {
      ...
      if (isEmpty(this.filterDocumentType)) {
        documentTypeList = this.documentTypeList.map(
          (documentType) => documentType.codice
        );
      }
      ...
      try {
        ...
        if (this.filterEpisodeType) {
          ...
          let { data: iEpisodes } = await getEpisodesList(
            this.taxCode,
            payload
          );
          ...
          let { data: eEpisodes } = await getEpisodesList(
            this.taxCode,
            payload
          );
          ...
          this.totalEpisodesCount =
            this.recoveryEpisodesCount + this.psEpisodesCount;
        }
      } catch (error) {
        let message =
          "Non è stato possibile recuperare la lista degli episodi.";
    ...
    async loadEpisodeDocumentList(episode) {
      ...
      try {
        let { data: documents } = await getEpisodesDocumentList(
          this.taxCode,
          idEpisode,
          { params }
        );
        ...
        let message =
          "Non è stato possibile recuperare la lista dei documenti per l'episodio selezionato.";
        apiErrorNotify({ error, message });
      }
