  ...
  <csi-page padding>
            ...
            <q-card-section class="q-pt-none">
              <q-list class="q-mt-md">
                ...
                <q-item class="no-padding q-mb-md">
                  <q-item-section>
                    <q-item-label caption class="text-black"
                      >Validato il</q-item-label
                    >
                    <q-item-label class="text-bold">{{
                      validationDate | date | empty
                    }}</q-item-label>
                  </q-item-section>
                </q-item>
    ...
    <div class="q-my-md" v-else>
      <csi-banner type="info">
        ...
        <template v-else> Nessun documento disponibile </template>
      </csi-banner>
    </div>
  </csi-page>
</template>
...
import { getUrlFromBase64 } from "src/services/documents/business-logic";
...
export default {
  ...
  computed: {
    ...
    typeName() {
      let documentType = this.documentTypeList?.find(
        (type) => type.codice === this.typeCode
      );
  ...
  async created() {
    ...
    try {
      ...
      if (error.response.status === 500) {
        ...
        else if (errorDetail[0]?.chiave === "CC_ER_155") this.withoutPSS = true;
      } else {
