    ...
    <div class="row items-stretch q-col-gutter-md" v-else>
      ...
      >
        <div class="text-h6 q-mb-md">
          Referto n. {{ document.id_documento_ilec ?? index }}
        </div>
        ...
        <template v-else>
          <div class="text-bold text-caption q-mb-sm"></div>

          <csi-pdf-viewer
            :src="document.url_documento"
            :iframe-classes="['full-width']"
            :iframe-styles="{ minHeight: '600px' }"
            :file-name="'referto_' + document.id_documento_ilec"
          />
        </template>
      </div>
    </div>
  ...
  async created() {
    ...
    for (let i = 0; i < this.documentListLength; i++) {
      ...
      if (isPersonal) {
        ...
        let idRepository =
          metadata?.id_repository_cl ?? metadata?.identificativo_repository;
        let payload = {
