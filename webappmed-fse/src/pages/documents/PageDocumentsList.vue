...
<template>
  <csi-page padding>
    ...
    <csi-banner v-if="hasDocuments" actions-inline type="info">
      È possibile selezionare più referti per confrontarli in un'unica pagina
      <template v-slot:action>
        <csi-buttons>
          <csi-button outline @click="compareDocuments"
            >Confronta referti</csi-button
          >
        </csi-buttons>
      </template>
    </csi-banner>
...
export default {
    ...
    documentTypeList() {
      ...
      categoryList.forEach((category) => {
        ...
        docTypes.forEach((type) => {
          ...
          if (
            category.codice_categoria === DOCUMENT_CATEGORY_MAP.FSE &&
            excludedDocTypes.includes(type.codice)
          ) {
            return;
          }
  ...
  methods: {
    async loadDocumentList() {
      ...
      if (this.patientHasConsents) {
        ...
        let payload = {
          ...
          offset: this.offset,
          tipo_medico:
            this.filterParams.tipo_medico ?? DOCUMENT_DOCTOR_TYPE_MAP.BOTH,
        };
        try {
          let { data: documents } = await getDoctorReports(
            this.patientTaxCode,
            payload
          );
        ...
        for (let i = 0; i < documentList.length; i++) {
          ...
          if (document.metadati_documento?.oscura_scarico_cittadino === "S") {
            ...
            try {
              let { data: mediationInfo } = await getDocumentMediationInfo(
                this.patientTaxCode,
                id,
                { params }
              );
    ...
    onSelectDocument(isSelected, document) {
      ...
      if (isSelected) {
        if (index < 0) this.selectedDocumentsList.push(document);
      } else {
