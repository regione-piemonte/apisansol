...
<template>
  <div class="page-report-images">
    <csi-banner class="q-mb-lg" type="info">
      Qualora l’Azienda Sanitaria e/o l’Unità Operativa indicate nel referto
      differiscano rispetto a quelle indicate nell’elenco episodi, fa fede
      l’indicazione riportata nel referto.
    </csi-banner>
    ...
    <template v-else>
            ...
            <template v-slot:body-cell-action="props">
              <q-td :props="props">
                <a
                  class="csi-link"
                  href="#"
                  @click.prevent="showImage(props.row)"
                >
                  Visualizza immagine</a
                >
              </q-td>
            </template>
                ...
                <q-card bordered flat>
                  <q-card-section class="q-pa-sm">
                    ...
                    <q-item class="no-padding" v-if="canShowImage">
                      <q-item-section>
                        <q-item-label class="text-caption"
                          >Viewer a bassa risoluzione</q-item-label
                        >
                        <q-item-label
                          ><a
                            class="csi-link"
          ...
          <q-item>
            <q-item-section>
              <q-item-label caption class="text-black"
                >Stato pacchetto</q-item-label
              >

              <q-item-label class="text-bold">{{
                imageStatusLabel | empty
              }}</q-item-label>
      ...
      <q-dialog v-model="showImageInfoDialog" persistent>
        <q-card style="max-width: 800px">
          ...
          <q-card-section>
            <p>
              Le immagini Dicom visualizzate
              <strong>non sono destinate ad uso diagnostico</strong>.<br />
              Il presente viewer può essere utilizzato solo come software a
              scopo di consultazione, ricerca o insegnamento. <br />
              <strong>Non può essere destinato ad uso diagnostico</strong> ed
              utilizzato a fini clinici e/o per la cura del paziente. Non è un
              dispositivo medico con marchio CE.
            </p>

            <csi-buttons class="q-mt-md">
              <csi-button @click="goToImageDetail" :loading="isSettingAudit"
                >Accetta</csi-button
              >
              <csi-button v-close-popup color="negative" outline
                >Rifiuta</csi-button
              >
            </csi-buttons>
          </q-card-section>
  ...
  methods: {
    ...
    async getImageStatus() {
      ...
      let params = {
        ...
        cod_documento_dipartimentale:
          this.metadata?.codice_documento_dipartimentale,
      ...
      try {
        ...
        let message = "Non è stato possibile verificare lo stato del pacchetto";
    ...
    async onBookImage() {
      ...
      try {
        ...
        let message = "non è stato possibile prenotare il pacchetto immagini";
        apiErrorNotify({ error, message });
      }
    ...
    async onDownloadImage() {
      ...
      try {
        ...
        let message = "Non è stato possibile scaricare il pacchetto";
        apiErrorNotify({ error, message });
      }
