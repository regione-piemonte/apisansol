  ...
  <div class="page-report-images">
    ...
    <template v-else-if="documentUrl">
      <csi-banner class="q-mb-md" type="warning">
        Se non vedi il tuo referto è possibile che siano da impostare
        correttamente alcuni parametri del tuo browser.
        <a class="csi-link" target="_blank">Clicca qui</a> per la tua guida.
        <br />È comunque possibile scaricarlo sul tuo pc cliccando
        <a
          :href="documentUrl"
          class="csi-link"
      ...
      <csi-banner
        ...
        type="warning"
      >
        Questo referto contiene <strong>immagini</strong> da visualizzare o
        scaricare
        <template #action>
      ...
      <div class="q-my-lg">
        <csi-pdf-viewer
          :file-name="'referto_' + id"
          :iframe-classes="['full-width']"
    ...
    <template v-else>
      <csi-banner type="negative">
        Non è stato possibile recuperare il documento
      </csi-banner>
    </template>
  </div>
</template>
...
export default {
  ...
  async created() {
    ...
    if (this.isPersonal) {
      ...
      let idRepository =
        this.metadata?.id_repository_cl ??
        this.metadata?.identificativo_repository;
      let payload = {
        firmato: "S",
