...
<template>
  <csi-page class="page-roles-list" padding>
    ...
    <template v-else-if="!qrCode">
      <csi-banner type="negative"> QRCode non valido </csi-banner>
    </template>

    <template v-else-if="!document">
      <fse-profile-choice
        :confirming="isLoadingDocument"
        @on-confirm="showDocument"
      />
    </template>

    <template v-else>
      <csi-pdf-viewer
        :iframe-styles="{ height: '100vh' }"
        :src="document"
        file-name="documento"
      />
    </template>
...
import { showDocumentFromQRCode } from "src/services/api";
