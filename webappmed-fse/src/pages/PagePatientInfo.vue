...
<template>
  <csi-page class="page-patient-info">
    <csi-page-title :back="HOME" class="q-mb-lg"
      >Scheda Paziente {{ patientTaxCode }}</csi-page-title
    >
          ...
          <q-card-section>
            <q-item class="q-py-md q-px-none" dense>
              ...
              <q-item-section>
                <q-item-label class="text-h6"
                  ><strong>{{ fullName }}</strong></q-item-label
                >
                <q-item-label caption class="text-black">
                  {{ gender === "F" ? "Nata" : "Nato" }} il:
            ...
            <template v-if="doctor">
              ...
              >
                ...
                <q-item-section>
                  <q-item-label class="text-h6"
                    ><strong>{{ doctorFullname | empty }}</strong></q-item-label
                  >
                  <q-item-label caption class="text-black"
                    >Dal 19/01/2020</q-item-label
                  >
                  <q-item-label
                    caption
                    class="text-black q-pt-md csi-wrap-word"
                    v-if="doctorEmail"
                  >
      ...
      <div class="col-lg col-12">
        ...
        >
          <q-route-tab
            ...
            v-show="route.visibile"
            :key="route.name"
            :label="route.label"
            :name="route.name"
            :to="route.url"
            exact
            @click="setRouteAudit(route.auditCode)"
          />
        </q-tabs>
...
export default {
  ...
  computed: {
      ...
      return [
        ...
        },
        {
          ...
          visibile:
            !this.hidePSS &&
            this.hasFse &&
            (this.hasConsents || this.isEmergencySystem),
        },
        {
    ...
    doctorFullname() {
      if (this.doctor) {
        return `Dott. ${startCase(this.doctor.nome)}  ${startCase(
          this.doctor.cognome
        )}`;
      }
      return "";
    ...
    doctorIcon() {
      ...
      let doctorIcon =
        doctorGender === "F" ? "avatar-medico-donna" : "avatar-medico-uomo";
  ...
  async created() {
    ...
    if (!this.doctorInfo && this.isPiedmontPatient) {
      try {
        let { data: doctor } = await getMmgInfo(this.patientTaxCode, {
          params,
        });

        if (doctor?.ha_medico === "SI") {
          this.$store.dispatch("setPatientDoctor", { doctor });
        } else if (doctor?.ha_medico === "NO") this.hasNoDoctor = true;
      } catch (error) {
        let message =
          "non è stato possibile recuperare i dati del medico curante";
  ...
  methods: {
    ...
    async handleNavigation(to, from, next) {
      // Quando la route target è il dettaglio paziente vuol dire che si tratta del primo accesso
