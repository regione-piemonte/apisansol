  ...
  <csi-page>
    <div class="row justify-center">
      <csi-banner class="q-mb-lg col-12 col-md-8" type="fse">
        Per accedere al fascicolo in qualità di soggetto abilitato alla
        consultazione dei dati e documenti clinico-sanitari disponibili nel FSE,
        ai sensi del DPCM n.178/2015 e s.m.i., deve indicare il Ruolo ed il
        Regime cui è coinvolto effettivamente nel processo di cura in atto.
        <strong
          >La specifica del motivo di accesso è obbligatoria per poter andare
          avanti.</strong
        >
      </csi-banner>
    </div>
    ...
    <template v-else-if="systemList.length <= 0">
      <csi-banner type="warning">
        Non è stato possibile recuperare la lista dei regimi.
      </csi-banner>
    </template>
      ...
      >
        ...
        <template v-slot:before>
          ...
          <template v-if="isMobile">
            <q-item key="header" class="q-py-none text-h1 text-black"
              >Regime</q-item
            >
          ...
          <template v-else>
            <div
              ref="fseSystemMenuContainer"
              v-show="!isMobile"
              class="q-pa-md row fse-system-menu-container justify-center"
              style="margin: 0 auto"
            >
              <q-list
                ref="fseSystemMenuList"
                :class="animationComplete ? 'col-12' : 'col-3'"
              >
                <q-item
                  key="header"
                  class="q-pa-none text-h1 text-black fse-system-item"
                  >Regime</q-item
                >
                <div
                ...
                >
                  <fse-system-menu-item
                    class="q-my-md"
                    :system="system"
                    @on-select="onSelectedSystemDesktop"
                  />
                </div>
              </q-list>
            </div>
...
export default {
  ...
  async created() {
    ...
    if (isEmpty(this.systemList)) {
      try {
        ...
        if (this.systemSelected) this.scrollToSelectedSystem();
      } catch (error) {
        let message = "Non è stato possibile recuperare la lista dei regimi";
        apiErrorNotify({ error, message });
      }
    }
    // Se il regime è stato selezionato o siamo su schermo mobile non attiviamo le animazioni
  ...
  methods: {
    ...
    scrollToSelectedSystem() {
      let seletedId = this.systemList?.find(
        (s) => s.codice == this.systemSelected?.codice
      );
    ...
    onSelectedSystemDesktop(system) {
      ...
      if (!this.animationComplete) {
        const container =
          this.$refs.fseSystemMenuContainer.getBoundingClientRect().left;
    ...
    scrollToMenuItem(index) {
      ...
      if (card) scrollHorizontalToElement(card);
    },
    getSystemIcon(code) {
