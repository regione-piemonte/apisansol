...
<template>
  <csi-page padding>
    ...
    <template v-else>
      <fse-profile-choice
        :confirming="isLoadingPatientInfo"
        :role="userRole"
        hide-role-list
...
export default {
  ...
  async created() {
    ...
    //Salviamo il token nello storage perché ci servirà per tutte le chiamate
    ...
    try {
      ...
      if (error.response.status === 500) {
        this.tokenError =
          "Non è stato posssibile recuperare i dati dell'utente. Rieseguire l'accesso.";
      }
    }
  ...
  methods: {
    ...
    async goToConsentsActivation() {
      ...
      await this.$store.dispatch(
        ...
        this.patientInfo.tipo_documento
      );
