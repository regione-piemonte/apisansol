  ...
  <csi-page padding>
      ...
      >
        ...
        >
          ...
          <div class="row q-col-gutter-md items-center justify-between">
            <div class="col-12 col-md">
              <q-toggle v-model="isNoticeShown"
                >All'assistito è stata mostrata l'informativa</q-toggle
              >
            </div>

            <div class="col-12 col-md-auto">
              <csi-buttons between>
                <csi-button :disabled="!isNoticeShown" @click="onStepNoticeNext"
                  >Conferma</csi-button
                >
        ...
        >
          <div class="text-h1 q-mb-md">
            <span
              class="fse-consent-consultation-click"
              @click="onConsentsConsultationClick()"
            >
              Consenso alla consultazione <q-icon name="info"></q-icon>
            </span>
          </div>
          <q-toggle v-model="isConsentAcepted" disable
            >L'assistito acconsente alla consultazione del Fascicolo da parte
            degli operatori sanitari e sociosanitari
          </q-toggle>

          <csi-banner type="info" class="q-my-md">
            Se il soggetto è minorenne, occorre indicare il codice fiscale del
            genitore o tutore
          </csi-banner>
          ...
          <div class="q-gutter-sm">
            <q-radio
              v-model="parentType"
              :val="AUDIT_CODES.CONSENTS_TAXCODE_PARENT"
              label="Genitore"
            />
            <q-radio
              v-model="parentType"
              :val="AUDIT_CODES.CONSENTS_TAXCODE_TUTOR"
              label="Tutore"
            />
          </div>

          <q-input
            v-model="parentTaxCode"
            class="q-mt-md"
            :disable="!parentType"
        ...
        >
          ...
          <div>
            <ul>
              <li>
                L'assistito ha preso visione dell'informativa sul trattamento
                dei dati personali
              </li>
              <li>
                L'assistito ha scelto di dare il consenso alla consultazione del
                Fascicolo da parte degli operatori sanitari e sociosanitari
              </li>
            </ul>
          </div>

          <csi-buttons between class="q-mt-lg">
            <csi-button :loading="isSendingConsent" @click="onStepSummaryNext"
              >Conferma</csi-button
            >
            <csi-button outline @click="onStepPrevious">Indietro</csi-button>
            <csi-button :to="HOME" outline>Annulla</csi-button>
    ...
    <template v-else>
      <csi-banner type="positive">
        L'assistito ha dato il consenso alla consultazione del Fascicolo
        Sanitario da parte degli operatori sanitari e sociosanitari
      </csi-banner>
      <p></p>
      <csi-banner type="warning">
        <p>
          <b>Importante:</b>
          Prima di accedere al Fascicolo Sanitario Elettronico occorre stampare
          e far firmare all'assistito il modulo che riepiloga le operazioni le
          operazioni effettuate e conservarlo.
        </p>
      </csi-banner>

      <csi-buttons class="q-mt-lg">
        <csi-button :to="PATIENT_INFO">Vai al fascicolo</csi-button>
        <csi-button outline @click="printConsents" :loading="isPrinting"
          >Stampa</csi-button
        >
      </csi-buttons>
    </template>
    ...
    <fse-consents-consultation-dialog
      v-model="showConsentsConsultationDialog"
    />
  </csi-page>
</template>
...
export default {
  ...
  async created() {
    ...
    let regionCode =
      this.consents?.identificativo_informativa_corrente?.substring(0, 3) ??
      "010";
    ...
    try {
      ...
      let message = "Non è stato possibile recuperare l'informativa";
  ...
  methods: {
    ...
    async onStepSummaryNext() {
      ...
      console.log("consents", this.consents);
      let parentTaxCode = !isEmpty(this.parentTaxCode)
        ? this.parentTaxCode
        : this.consents?.identificativo_assistito_consenso;
      let payload = {
        presa_in_carico: true,
        struttura_utente: "------",
        tipo_attivita: "CREATE",
        contesto_operativo: "CONSENT",
        identificativo_assistito_consenso:
          this.consents?.identificativo_assistito_consenso,
        ...
        identificativo_informativa:
          this.consents?.identificativo_informativa_corrente,
      ...
      try {
        ...
        let message = "Non è stato possibile modificare i consensi";
        apiErrorNotify({ error, message });
      }
    ...
    async setParentAudit() {
      //Audit per indicare se il cf è di tutore o genitore
