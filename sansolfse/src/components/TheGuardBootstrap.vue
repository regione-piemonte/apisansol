...
export default {
  ...
  async created() {
    ...
    let taxCode = delegatorSelected
      ? delegatorSelected.codice_fiscale_delega
      : this.user?.cf;
    ...
    if (taxCode) {
      ...
      this.$store.dispatch("loadTagList", { taxCode });
      await this.$store.dispatch("loadCategoryList", { taxCode });
    }
