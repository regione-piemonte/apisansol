  ...
  <div class="lms-the-guard-enrollment-2">
    ...
    <template v-if="['012'].includes(code)">
      <q-banner class="bg-yellow-2" rounded>
        ...
        <div class="text-body1">
          Il processo del trasferimento dei dati del Fascicolo Sanitario
          Elettronico dalla precedente alla nuova regione di assistenza è stato
          completato.
        </div>
...
export default {
  ...
  computed: {
    ...
    enrollmentUrl() {
      return "/la-mia-salute/arruolamento/#/?servizio=FSEDOC";
    },
    enrollmentUrlDelegator() {
      let url = "/la-mia-salute/arruolamento/#/?servizio=FSEDOC";
  ...
  methods: {
    onActivate() {
      const url = "/la-mia-salute/arruolamento/#/?servizio=FSEDOC";
