...
export const loadTagList = async (context, { taxCode }) => {
  let { data: tagList } = await getTagList(taxCode);
...
export const loadCategoryList = async (context, { taxCode }) => {
  let params = { codice_app_verticale: APP_CODE_MAP.FSE };
  let { data: categoryList } = await getCategoryList(taxCode, { params });
  context.commit("SET_CATEGORY_LIST", { categoryList });
};

export const setExemptionInfo = (context, { exemptionInfo }) => {
