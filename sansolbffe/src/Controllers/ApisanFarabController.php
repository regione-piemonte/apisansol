...
class ApisanFarabController
{
    ...
    {
        $http = new Http();
        $http->setAuth(APISANFARAB_USER, APISANFARAB_PASS);
        if (USE_PROXY) $http->setProxy(PROXY);

        $request = clone $req;
        $uri = $request->getUri();
        $uri = $uri->withScheme(APISANFARAB_SCHEME);
        $uri = $uri->withHost(APISANFARAB_HOST);
        $uri = $uri->withPort(APISANFARAB_PORT);
