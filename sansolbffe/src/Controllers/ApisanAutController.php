...
class ApisanAutController
{
    ...
    {
        $http = new Http();
        $http->setAuth(APISANAUT_USER, APISANAUT_PASS);
        if (USE_PROXY) $http->setProxy(PROXY);

        $request = clone $req;
        $uri = $request->getUri();
        $uri = $uri->withScheme(APISANAUT_SCHEME);
        $uri = $uri->withHost(APISANAUT_HOST);
        $uri = $uri->withPort(APISANAUT_PORT);
