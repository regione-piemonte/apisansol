...
class ApisanCeliachiaController
{
    ...
    {
        $http = new Http();
        $http->setAuth(APISANCELIACHIA_USER, APISANCELIACHIA_PASS);
        if (USE_PROXY) $http->setProxy(PROXY);

        $request = clone $req;
        $uri = $request->getUri();
        $uri = $uri->withScheme(APISANCELIACHIA_SCHEME);
        $uri = $uri->withHost(APISANCELIACHIA_HOST);
        $uri = $uri->withPort(APISANCELIACHIA_PORT);
