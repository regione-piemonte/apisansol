...
class NotifyMessageBrokerController
{
    ...
    public function proxy($req, $res, $args)
    {
        ...
        $request = clone $req;
        $uri = $request->getUri();
        $uri = $uri->withScheme(NOTIFY_MESSAGE_BROKER_SCHEME);
        $uri = $uri->withHost(NOTIFY_MESSAGE_BROKER_HOST);
        $uri = $uri->withPort(NOTIFY_MESSAGE_BROKER_PORT);
