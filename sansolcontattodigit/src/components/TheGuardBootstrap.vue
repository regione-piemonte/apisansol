...
import {
  ...
  getNotifyContacts,
  getDelegatorList,
  getEnrollmentInfo, getDoctorInfo,  getPatientEnablingInfo,
  // getUserInfo,
} from "src/services/api";
import { getEnrollmentConsent, getUserInfo } from "../services/api";
...
import {hasEnrollmentBlock} from "src/services/business-logic";
...
export default {
  ...
  computed: {
    ...
    documentHiddenDialogInfo(){
      return this.$store.getters["getDocumentHiddenDialogInfo"];
    }
  },
  async created() {
    this.isLoading = true;
    ...
    try {
      ...
      try {
        ...
        await this.$store.dispatch("setEnrollmentInfo", { enrollmentInfo });
      } catch (e) {
        console.error(e);
      }

      try {
        let { data: enrollmentConsent } = await userEnrollmentConsentPromise;
    ...
    //CHIAMATE DI CONTATTO DIGITALE IN CASO NON SIA BLOCCATO

    if(!this.isEnrollmentCodeBlocker){
      let taxCode = this.$store.getters["getTaxCode"]

      await this.$store.dispatch('setUserDoctor', taxCode);
      ...
      if (this.userDoctor) {
        ...
        try {
          let response = await getPatientEnablingInfo(taxCode, {params})
    ...
    this.$store.dispatch('setDocumentHiddenDialogInfo', {status: this.documentHiddenDialogInfo})
    this.isLoading = false;
  }
};
</script>
