...
export const SET_EPISODE_TYPE_LIST = (state, { episodeTypeList }) => {
  state.episodeTypeList = episodeTypeList;
};
...
export const SET_USER_DOCTOR = (state, {info}) => {
  state.userDoctor = info;
};

export const SET_NO_DOCTOR_ERR = (state, value) => {
  state.noDoctorErr = value;
};
export const SET_USER_ENABLING_INFO = (state, {info}) => {
  state.enablingInfo = info;
};

export const SET_DOCUMENT_LIST = (state, {documents}) => {
  state.documentList = documents;
};

export const SET_DOCUMENT_HIDDEN_DIALOG = (state, {status}) => {
  state.documentHiddenDialog = status;
}
