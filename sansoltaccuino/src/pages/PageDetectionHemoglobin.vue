...
<template>
  <tac-page padding menu>
    <div class="row items-center q-col-gutter-x-md q-col-gutter-y-lg">
      <div class="col-auto">
        <q-icon
          name="img:/statics/la-mia-salute/icone/emoglobina.svg"
          size="md"
        />
      </div>
    ...
    <div class="q-mt-xl">
      ...
      <q-tab-panels v-model="visualizationSelected" class="bg-transparent">
            ...
            <q-card>
              <q-card-section
                v-for="[
                  measure,
                  list
                ] in detectionHemoglobinListGroupedByMeasure"
                :key="'hemoglobin--' + measure"
              >
                <div class="text-bold">Emoglobina - {{ measure }}</div>
        ...
        <q-tab-panel :name="VISUALIZATION_MAP.TABLE">
          <q-card>
            <tac-table-detection
              :detection-list="detectionHemoglobinList"
              @delete="onDelete"
            />
          </q-card>
        </q-tab-panel>
...
import TacDetectionHemoglobinCreateDialog from "../components/TacDetectionHemoglobinCreateDialog";
...
export default {
  ...
  computed: {
    ...
    detectionHemoglobinListGroupedByMeasure() {
      return getDetectionEntryListByMeasure(this.detectionHemoglobinList);
    },
    detectionList() {
      return [...this.detectionHemoglobinList];
    },
    ruleBeforeEndDate() {
      return v =>
        v < this.endDate_ ||
  ...
  methods: {
    ...
    async loadDetectionList() {
      ...
      try {
        ...
        this.detectionHemoglobinList = hemoglobinDetectionList?.lista ?? [];
