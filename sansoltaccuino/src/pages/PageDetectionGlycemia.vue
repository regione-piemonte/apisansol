...
<template>
  <tac-page padding menu>
    <div class="row items-center q-col-gutter-x-md q-col-gutter-y-lg">
      <div class="col-auto">
        <q-icon
          name="img:/statics/la-mia-salute/icone/glicemia.svg"
          size="md"
        />
      </div>
      ...
      <q-tab-panels v-model="visualizationSelected" class="bg-transparent">
        ...
        <q-tab-panel :name="VISUALIZATION_MAP.GRAPH">
          <template v-if="!isLoading">
            <q-card>
              <q-card-section
                v-for="[measure, list] in detectionGlycemiaListGroupedByMeasure"
                :key="'glycemia--' + measure"
              >
                <div class="text-bold">Glicemia - {{ measure }}</div>
        ...
        <q-tab-panel :name="VISUALIZATION_MAP.TABLE">
          <q-card>
            <tac-table-detection
              :detection-list="detectionGlycemiaList"
              @delete="onDelete"
            />
          </q-card>
        </q-tab-panel>
...
import TacDetectionGlycemiaCreateDialog from "../components/TacDetectionGlycemiaCreateDialog";
...
export default {
  ...
  computed: {
    ...
    detectionGlycemiaListGroupedByMeasure() {
      return getDetectionEntryListByMeasure(this.detectionGlycemiaList);
    },
    detectionList() {
      return [...this.detectionGlycemiaList];
    },
    ruleBeforeEndDate() {
      return v =>
        v < this.endDate_ ||
  ...
  methods: {
    ...
    async loadDetectionList() {
      ...
      try {
        ...
        this.detectionGlycemiaList = glycemiaDetectionList?.lista ?? [];
