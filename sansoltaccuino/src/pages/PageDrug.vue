...
<template>
  <tac-page padding menu>
    <div class="row items-center q-col-gutter-x-md q-col-gutter-y-lg">
      <div class="col-auto">
        <q-icon name="img:/statics/la-mia-salute/icone/farmaci.svg" size="md" />
      </div>
      ...
      <q-card class="relative-position">
        ...
        >
          <template #body-cell-date="props">
            <q-td :props="props">
              {{ props.row.data_assunzione | datetime }}
            </q-td>
          </template>
...
import { deleteDrug, getDrugList } from "../services/api";
import { apiErrorNotify, apiErrorNotifyDialog } from "../services/utils";
import TacDrugCreateDialog from "../components/TacDrugCreateDialog";
import TacTableGridItemDrug from "../components/TacTableGridItemDrug";
  ...
  methods: {
    ...
    onDelete(row) {
      this.$q
        .dialog({
          title: "Conferma",
          message: "Stai per rimuovere un farmaco annotato, vuoi procedere?",
        ...
        .onOk(async () => {
          ...
          try {
            let { data } = await deleteDrug(taxCode, notebookId, drugId);
            this.loadDrugList();
          } catch (err) {
            let message =
              "Non è stato possibile rimuovere il farmaco selezionato";
            apiErrorNotifyDialog({ err, message });
          }
        });
    }
