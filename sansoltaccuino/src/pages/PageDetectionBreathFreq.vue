...
<template>
  <tac-page padding menu>
    <div class="row items-center q-col-gutter-x-md q-col-gutter-y-lg">
      <div class="col-auto">
        <q-icon name="img:/statics/la-mia-salute/icone/polmoni.svg" size="md" />
      </div>
    ...
    <div class="q-mt-xl">
      ...
      <q-tab-panels v-model="visualizationSelected" class="bg-transparent">
            ...
            <q-card>
              <q-card-section
                v-for="[
                  measure,
                  list
                ] in detectionBreathFreqListGroupedByMeasure"
                :key="'breath-freq--' + measure"
              >
                <div class="text-bold">Frequenza - {{ measure }}</div>
        ...
        <q-tab-panel :name="VISUALIZATION_MAP.TABLE">
          <q-card>
            <tac-table-detection
              :detection-list="detectionBreathFreqList"
              @delete="onDelete"
            />
          </q-card>
        </q-tab-panel>
...
import TacDetectionBreathFreqCreateDialog from "../components/TacDetectionBreathFreqCreateDialog";
...
export default {
  ...
  computed: {
    ...
    detectionBreathFreqListGroupedByMeasure() {
      return getDetectionEntryListByMeasure(this.detectionBreathFreqList);
    },
    detectionList() {
      return [...this.detectionBreathFreqList];
    },
    ruleBeforeEndDate() {
      return v =>
        v < this.endDate_ ||
  ...
  methods: {
    ...
    async loadDetectionList() {
      ...
      try {
        ...
        this.detectionBreathFreqList = breathFreqDetectionList?.lista ?? [];

        // let { data } = await getDetectionList(taxCode, notebookId, { params });
        // this.detectionGroupList = data;
      } catch (err) {
        let message =
          "Non è stato possibile caricare le informazioni sulle frequenze respiratorie";
