...
<template>
  <tac-page padding menu>
    <div class="row items-center q-col-gutter-x-md q-col-gutter-y-lg">
      <div class="col-auto">
        <q-icon
          name="img:/statics/la-mia-salute/icone/raffreddore.svg"
          size="md"
        />
      </div>
...
import { deleteBulkSymptom, getSymptomList } from "../services/api";
import { apiErrorNotify, apiErrorNotifyDialog } from "../services/utils";
import TacSymptomCreateDialog from "../components/TacSymptomCreateDialog";
import TacTableGridItemSymptom from "../components/TacTableGridItemSymptom";
  ...
  methods: {
    ...
    onDelete(row) {
      this.$q
        .dialog({
          title: "Conferma",
          message: "Stai per rimuovere un sintomo rilevato, vuoi procedere?",
        ...
        .onOk(async () => {
          ...
          try {
            let { data } = await deleteBulkSymptom(
              taxCode,
              notebookId,
              payload
            );
            ...
            let message =
              "Non è stato possibile rimuovere i sintomi selezionati";
            apiErrorNotifyDialog({ err, message });
          }
        });
    }
