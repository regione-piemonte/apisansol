...
<template>
  <tac-page padding menu>
    <div class="row items-center q-col-gutter-x-md q-col-gutter-y-lg">
      <div class="col-auto">
        <q-icon name="img:/statics/la-mia-salute/icone/dolore.svg" size="md" />
      </div>
      ...
      <q-card class="relative-position">
        ...
        >
          <template #body-cell-start-date="props">
            <q-td :props="props">
              {{ props.row.data_inizio | datetime }}
            </q-td>
          </template>

          <template #body-cell-end-date="props">
            <q-td :props="props">
              {{ props.row.data_fine | datetime }}
            </q-td>
          </template>
...
import { deleteBulkPain, getPainList } from "../services/api";
import { apiErrorNotify, apiErrorNotifyDialog } from "../services/utils";
import TacPainCreateDialog from "../components/TacPainCreateDialog";
import TacTableGridItemPain from "../components/TacTableGridItemPain";
...
const TABLE_COLS = [
  ...
  },
  {
    name: "intensity",
    label: "Intensità",
    field: row => row.intensita?.descrizione,
    align: "left",
    sortable: false
  },
  {
  ...
  methods: {
    ...
    onDelete(row) {
      this.$q
        .dialog({
          title: "Conferma",
          message: "Stai per rimuovere una dolore rilevato, vuoi procedere?",
        ...
        .onOk(async () => {
          ...
          try {
            let { data } = await deleteBulkPain(taxCode, notebookId, payload);
            this.loadPainList();
          } catch (err) {
            let message =
              "Non è stato possibile rimuovere i dolori selezionati";
            apiErrorNotifyDialog({ err, message });
          }
        });
    }
