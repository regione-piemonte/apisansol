...
<template>
  <tac-page padding menu>
    <div class="row items-center q-col-gutter-x-md q-col-gutter-y-lg">
      <div class="col-auto">
        <q-icon
          name="img:/statics/la-mia-salute/icone/vertebre.svg"
          size="md"
        />
      </div>
      ...
      <q-tab-panels v-model="visualizationSelected" class="bg-transparent">
        ...
        <q-tab-panel :name="VISUALIZATION_MAP.GRAPH">
          <template v-if="!isLoading">
            <q-card>
              <q-card-section
                v-for="[measure, list] in detectionOxymetryListGroupedByMeasure"
                :key="'oxymetry--' + measure"
              >
                <div class="text-bold">Ossimetria - {{ measure }}</div>
        ...
        <q-tab-panel :name="VISUALIZATION_MAP.TABLE">
          <q-card>
            <tac-table-detection
              :detection-list="detectionOxymetryList"
              @delete="onDelete"
            />
          </q-card>
        </q-tab-panel>
...
import TacDetectionOxymetryCreateDialog from "../components/TacDetectionOxymetryCreateDialog";
...
export default {
  ...
  computed: {
    ...
    detectionOxymetryListGroupedByMeasure() {
      return getDetectionEntryListByMeasure(this.detectionOxymetryList);
    },
    detectionList() {
      return [...this.detectionOxymetryList];
    },
    ruleBeforeEndDate() {
      return v =>
        v < this.endDate_ ||
  ...
  methods: {
    ...
    async loadDetectionList() {
      ...
      try {
        ...
        this.detectionOxymetryList = oxymetryDetectionList?.lista ?? [];
