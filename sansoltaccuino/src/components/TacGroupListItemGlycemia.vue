...
<template>
  <div class="tac-group-list-item-temperature">
    ...
    <template v-else>
      <tac-group-list-item
        title="Glicemia"
        icon="img:/statics/la-mia-salute/icone/glicemia.svg"
        addable
        graph
      ...
      >
        <template #text>
          <!-- TESTO STATO VUOTO -->
          <!-- ----------------------------------------------------------------------------------------------------- -->
          <template v-if="!detectionGlycemia">
          ...
          <template v-else>
            ...
            <div>
              <span class="text-bold">{{ glycemia | decimals | number }}</span>
              {{ glycemiaMeasure }}
            </div>
          </template>
        </template>
    ...
    <template v-if="isCreateDialogOpen">
      <tac-detection-glycemia-create-dialog
...
import { DETECTION_GLYCEMIA } from "../router/routes";
import { apiErrorNotify, apiErrorNotifyDialog } from "../services/utils";
import TacDetectionGlycemiaCreateDialog from "./TacDetectionGlycemiaCreateDialog";
...
const GROUP_CODE = GROUP_CODE_MAP.GLYCEMIA;

export default {
  name: "TacGroupListItemGlycemia",
  components: {
    TacGroupListItemSkeleton,
    TacDetectionGlycemiaCreateDialog,
    TacGroupListItem
  },
  props: {},
  data() {
  ...
  computed: {
    ...
    detectionModeLabel() {
      return this.detectionGlycemia?.modalita?.descrizione_nazionale;
    },
    date() {
      return this.detectionGlycemia?.data;
    },
    glycemia() {
      return this.detectionGlycemia?.valore_numerico;
    },
    glycemiaMeasure() {
      return this.detectionGlycemia?.unita_misura_codice;
    }
  },
  created() {
  ...
  methods: {
    async loadLastDetection() {
      ...
      let glycemiaParams = {
        ...params,
        descrittore: DESCRIPTOR_CODE_MAP.GLYCEMIA
      };
      let glycemiaPromise = getDetectionList(taxCode, notebookId, {
        params: glycemiaParams
      });

      this.isLoading = true;

      try {
        let { data: glycemiaDetectionList } = await glycemiaPromise;

        this.detectionGlycemia = glycemiaDetectionList?.lista?.[0];

        // let { data } = await getDetectionList(taxCode, notebookId, { params });
        // this.lastDetection = data[0];
      } catch (err) {
        let message =
          "Non è stato possibile caricare le informazioni sulla glicemia";
    ...
    onGraphClick() {
      this.$router.push(DETECTION_GLYCEMIA);
    }
  }
};
</script>
