...
<template>
  <div class="tac-group-list-item-breath-freq">
    <!-- CARICAMENTO -->
    <!-- ----------------------------------------------------------------------------------------------------------- -->
    ...
    <template v-else>
      <tac-group-list-item
        ...
        icon="img:/statics/la-mia-salute/icone/polmoni.svg"
        addable
        graph
      ...
      >
        <template #text>
          <!-- TESTO STATO VUOTO -->
          <!-- ----------------------------------------------------------------------------------------------------- -->
          <template v-if="!detectionBreathFreq">
          ...
          <template v-else>
            ...
            <div>
              <span class="text-bold">{{ freq | decimals | number }}</span>
              {{ freqMeasure }}
            </div>
          </template>
        </template>
    ...
    <template v-if="isCreateDialogOpen">
      <tac-detection-breath-freq-create-dialog
...
import { DETECTION_BREATH_FREQ } from "../router/routes";
import { apiErrorNotify, apiErrorNotifyDialog } from "../services/utils";
import TacDetectionBreathFreqCreateDialog from "./TacDetectionBreathFreqCreateDialog";
...
export default {
  name: "TacGroupListItemBreathFreq",
  components: {
    TacGroupListItemSkeleton,
    TacDetectionBreathFreqCreateDialog,
    TacGroupListItem
  },
  props: {},
  data() {
  ...
  computed: {
    ...
    detectionModeLabel() {
      return this.detectionBreathFreq?.modalita?.descrizione_nazionale;
    },
    date() {
      return this.detectionBreathFreq?.data;
    },
    freq() {
      return this.detectionBreathFreq?.valore_numerico;
    },
    freqMeasure() {
      return this.detectionBreathFreq?.unita_misura_codice;
    }
  },
  created() {
  ...
  methods: {
    async loadLastDetection() {
      ...
      let breathFreqParams = {
        ...params,
        descrittore: DESCRIPTOR_CODE_MAP.BREATH_FREQ
      };
      let breathFreqPromise = getDetectionList(taxCode, notebookId, {
        params: breathFreqParams
      });

      this.isLoading = true;

      try {
        let { data: breathFreqDetectionList } = await breathFreqPromise;

        this.detectionBreathFreq = breathFreqDetectionList?.lista?.[0];

        // let { data } = await getDetectionList(taxCode, notebookId, { params });
        // this.lastDetection = data[0];
      } catch (err) {
        let message =
          "Non è stato possibile caricare le informazioni sulla frequenza cardiaca";
    ...
    onGraphClick() {
      this.$router.push(DETECTION_BREATH_FREQ);
    }
  }
};
</script>
