...
<template>
  <q-dialog
    ...
    class="tac-detection-temperature-create-dialog"
  >
    <q-card>
      <q-form
        novalidate
      ...
      >
        <q-toolbar>
          <q-toolbar-title>
            Inserisci la temperatura rilevata
          </q-toolbar-title>
            ...
            <div class="col-12">
              <div class="row q-col-gutter-md">
                <div class="col">
                  ...
                  >
                    <template v-if="isTemperatureSelfSelectable" #after>
                      <div class="text-body1 text-black text-bold self-end">
                        {{ temperatureSelfSelectableValue }}
                      </div>
                    </template>
                  </q-input>
                </div>

                <template v-if="!isTemperatureSelfSelectable">
                  <div class="col-12 col-sm-6 col-md-4">
                    <q-select
                      v-model="temperatureMeasure"
                      :options="temperatureMeasureList"
...
export default {
  name: "TacDetectionTemperatureCreateDialog",
  ...
  computed: {
    ...
    temperatureMeasureList() {
      return this.measureUnitConfig.filter(
        m => m.descrittore_codice === DESCRIPTOR_CODE_MAP.TEMPERATURE
      );
    },
    isTemperatureSelfSelectable() {
      return this.temperatureMeasureList.length === 1;
    },
    temperatureSelfSelectableValue() {
      return this.temperatureMeasureList[0]?.unita_misura_codice ?? null;
    },
    isTemperatureSelfSelected() {
      return isNumber(this.temperature) && this.isTemperatureSelfSelectable;
    },
    ruleRequired() {
  ...
  created() {
    ...
    if (this.temperatureMeasureList?.length === 1) {
      this.temperatureMeasure = this.temperatureMeasureList[0]?.unita_misura_codice;
    }
  },
  watch: {
    temperature: {
      ...
      handler(newValue, oldValue) {
        if (!this.isTemperatureSelfSelectable) return;

        if (isNumber(newValue) && !isNumber(oldValue)) {
          this.temperatureMeasure = this.temperatureSelfSelectableValue;
          return;
        }

        if (!isNumber(newValue) && isNumber(oldValue)) {
          this.temperatureMeasure = null;
          return;
        }
      }
    }
  ...
  methods: {
    ...
    async onsubmit() {
      ...
      let payload = {
        ...
        lista: [
          {
            valore_numerico: this.temperature,
            data: datetime,
            unita_misura_codice: this.temperatureMeasure,
            gruppo_codice: GROUP_CODE,
            descrittore_codice: DESCRIPTOR_CODE_MAP.TEMPERATURE,
      ...
      try {
        ...
        // setLastSelectedMeasure(map.TEMPERATURE, this.temperatureMeasure);
