...
<template>
  <q-dialog
    ...
    class="tac-detection-glycemia-create-dialog"
  >
    <q-card>
      <q-form
        greedy
      ...
      >
        <q-toolbar>
          <q-toolbar-title>
            Inserisci la glicemia rilevata
          </q-toolbar-title>
            ...
            <div class="col-12">
              <div class="row q-col-gutter-md">
                ...
                <div class="col-12 col-sm-6 col-md-4">
                  <q-select
                    v-model="glycemiaMeasure"
                    :options="glycemiaMeasureList"
...
export default {
  name: "TacDetectionGlycemiaCreateDialog",
  ...
  computed: {
    ...
    glycemiaMeasureList() {
      return this.measureUnitConfig.filter(
        m => m.descrittore_codice === DESCRIPTOR_CODE_MAP.GLYCEMIA
      );
    },
    ruleRequired() {
  ...
  created() {
    ...
    if (this.glycemiaMeasureList?.length === 1) {
      this.glycemiaMeasure = this.glycemiaMeasureList[0]?.unita_misura_codice;
    }
  },
  methods: {
    close() {
    ...
    async onsubmit() {
      ...
      let payload = {
        ...
        lista: [
          {
            valore_numerico: this.glycemia,
            data: datetime,
            unita_misura_codice: this.glycemiaMeasure,
            gruppo_codice: GROUP_CODE,
            descrittore_codice: DESCRIPTOR_CODE_MAP.GLYCEMIA,
      ...
      try {
        ...
        // setLastSelectedMeasure(map.GLYCEMIA, this.glycemiaMeasure);
