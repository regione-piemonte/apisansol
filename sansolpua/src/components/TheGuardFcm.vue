    ...
    <template v-if="messaging">
      <q-dialog v-model="isUiPermissionsVisible" position="bottom">
        <q-card class="lms-fcm-permission-dialog">
          ...
          <q-card-section>
            <lms-buttons>
              <lms-button @click="onUiPermissionsGranted">
                Ok
              </lms-button>
...
export default {
  ...
  methods: {
    ...
    askPermissions() {
      // Se si tratta dell'utente di monitoraggio
      // => non mostriamo il popup per chiedere i permessi
      if (this.user?.cf === MONITORING_CF.SIMON) {
        console.debug("[FCM] Do not Ask UI permission to symon user");
        return;
      }
