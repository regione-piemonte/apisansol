    ...
    <q-card class="onboarding-dialog full-height" :class="{'minWidth' : $q.screen.gt.xs, 'heightCard': $q.screen.lt.xl}">
      <q-card-section class="no-padding full-height  ">
        ...
        >
          <q-carousel-slide
            class="no-padding"
            v-for="(card,index) in FSE_LIST_ITEMS"
            ...
            <div class="column no-wrap onboarding-dialog__slide q-pa-md  relative-position">
              ...
              <div class="row items-center justify-center ">
                <q-btn
                  class="col-auto"
                  v-for="(item, index) in FSE_LIST_ITEMS"
                  :key="index"
                  size="xs"
                  flat
                  round
...
<script>
    import {FSE_LIST_ITEMS} from "src/services/config";
...
name: "HomeFseDialog",
  ...
  computed:{
    ...
    isLastItem(){
      return this.slide === FSE_LIST_ITEMS.length -1
    },
    isFirstItem(){
      return this.slide === 0
    },
  ...
  methods:{
    ...
    closeDialog(){
      this.$emit('close-fse-dialog');
    },
    slideImage(index){
     return FSE_LIST_ITEMS[index]?.icon + '.svg'
    },
    previousSlice(){
