...
export const isMinor = birthDate => {
  return getDateDiff(new Date(), birthDate, "years") < 18;
};
export const vaccinationsList = associations =>{
 return  associations?.reduce((out, a) => {
   return [...out, ...a.vaccinazioni];
 }, [])
}

export const vaccinationsNames = vaccinations =>{
  return vaccinations?.map(v => v.descrizione).filter(v => !!v).join(', ');
}
