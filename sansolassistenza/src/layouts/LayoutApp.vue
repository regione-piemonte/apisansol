...
<template>
  <q-layout view="hHh lpr fff" class="lms-layout-app">
    ...
    <!-- ----------------------------------------------------------------------------------------------------------- -->
    <q-page-container>
      <template v-if="isUserMinorHidden">
...
export default {
  ...
  computed: {
    ...
    isUserMinorHidden() {
      let isUserMinorVisible= this.$route.meta?.isUserMinorVisible
      let isUserMinor = this.$store.getters["isUserMinor"];
      return isUserMinor && !isUserMinorVisible
    }
  },
  async created() {
    if (this.user) {
