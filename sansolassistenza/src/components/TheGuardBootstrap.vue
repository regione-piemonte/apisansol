...
export default {
  ...
  async created() {
    ...
    let assistanceConfiguratonPromise = getConfiguration()

    try {
      let { data: configuration } = await assistanceConfiguratonPromise;
      this.$store.dispatch("setAssistanceConfiguration", { configuration });
    } catch(e) {
      console.error(e)
    }
    ...
    if(IS_DEV || IS_TEST){
      try {
        let {data: asrList} = await asrPromise;
    ...
    try {
      ...
      //Carica la lista delle informazioni per gli applicativi
      if(this.user){
        try {
          let {data: appList} = await getUserApplicationsList(taxCode)
