...
<template>
  <q-page padding>
    <csi-page-title title="Esenzione annullata"
                    @back="goToHome"/>

    <template v-if="exemption && !isLoading">
      <q-alert color="positive" class="q-mt-lg q-mx-md">
        <div class="q-display-2 text-weight-bold">
          La richiesta di annullamento per {{exemption.motivazione_annullamento.descrizione}} è stata completata con
          successo. <br>
          Numero pratica {{exemption.numero_pratica}}. Cancellazione richiesta il {{today | format}}
        </div>
