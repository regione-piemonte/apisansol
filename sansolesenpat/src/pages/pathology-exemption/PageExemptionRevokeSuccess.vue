...
<template>
  <q-page padding>
    <csi-page-title title="Esenzione revocata"
                    @back="goToHome"/>

    <template v-if="exemption && !isLoading">
      <q-alert color="positive" class="q-mt-lg q-mx-md">
        <div class="q-display-2 text-weight-bold">
          La richiesta di revoca per {{exemption.motivazione_revoca.descrizione}} è stata completata con successo. <br>
          Numero pratica {{exemption.numero_pratica}}. Revoca richiesta il {{today | format}}
        </div>
        ...
        <div class="q-pt-md text-weight-light">
          ...
          Scrivi a assistenza.salute@csi.it specificando: il tuo recapito telefonico, il tuo codice fiscale, il problema che hai rilevato
        </div>
      </q-alert>
